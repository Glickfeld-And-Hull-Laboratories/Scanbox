...
% Last Modified by GUIDE v2.5 01-Dec-2015 08:44:33
...
%     [fn,pathname] = uigetfile;
    ...
    %------------------------------------------------
    % ADDED BY DMS, 12/1/15
    % Adjust the slider
    handles.vidA = [];
    set(handles.video_slider,'Min',1)
    set(handles.video_slider,'Max',2)
    set(handles.video_slider,'SliderStep',[1 1])
    set(handles.video_slider,'Value',1)
    set(handles.video_slider,'Visible','off')
    ...
    handles.fn = fn;
    set(handles.file_text,'String',handles.fn)
...
function loadvideobtn_Callback(hObject, eventdata, handles)
    % ADDED BY DMS, 12/1/15
    % Load the full movie
    disp('Loading and aligning the full movie...')
    fnvid = handles.fn(1:end-6);
    

    handles.vidA = nan*ones([1,size(handles.m),size(handles.T,1)]);
    for jj = 1:size(handles.T,1)
        z = double(sbxreadpacked(fnvid,jj-1,1));
        z2 = circshift(z,handles.T(jj,:));
        handles.vidA(1,:,:,jj) = z2;
    end
    
%     try
%         vid = sbxread(fnvid,1,size(handles.T,1));%info.max_idx);
%     catch
%         return
%     end
%     
%     % Align the full movie
%     disp('Aligning the movie...')
%     handles.vidA = sbxalignvideo_dms(vid,handles.T);
    
    % Adjust the slider
    set(handles.video_slider,'Min',1)
    set(handles.video_slider,'Max',size(handles.vidA,4))
    set(handles.video_slider,'SliderStep',[1/(size(handles.vidA,4)-1) 1/(size(handles.vidA,4)-1)])
    set(handles.video_slider,'Value',1)
    set(handles.video_slider,'Visible','on')
    disp('Video loaded!')
...
set(handles.im_flood,'Visible','off')
handles.xraypoint = [];
curxrayval = get(handles.xraychk,'Value');
if curxrayval == 0,
    set(handles.xraychk,'Value',1)
else
    set(handles.xraychk,'Value',0)
...
set(handles.xraychk,'Value',0)
txt = 'PRESS "ESC" TO EXIT POLYGON MODE!';
set(handles.file_text,'String',txt,'ForegroundColor','r')
...
while polygonchk == 1,
    ...
    % If user aborted by pressing ESC, h will be empty
    if isempty(h),
        % Break out of this loop
        polygonchk = 0;
        
        set(handles.file_text,'String',handles.fn,'ForegroundColor','k')
    else
        % Convert the polygon to a full mask
        polyxy = h.getPosition;
        newmask = poly2mask(polyxy(:,1),polyxy(:,2),size(handles.m,1),size(handles.m,2));
        
        % Exclude pixels that belong to other ROIs
        newmaskKeep = setdiff(find(newmask),find(handles.mask));
        newmask = zeros(size(newmask));
        newmask(newmaskKeep) = 1;
        
        % Update the mask
        % Commented by dms: handles = guidata(hObject);
        % Commented by dms: newmask = handles.floodmap < handles.npixels;
        handles.mask = handles.mask + (max(handles.mask(:))+1)*newmask;
...
if ~isempty(handles.vidA),
    nFrames = 5;
    curFrame = round(get(hObject,'Value'));
    
    if curFrame<=nFrames,
        curFrames = 1:nFrames;
    elseif curFrame>=(size(handles.vidA,4)-nFrames),
        curFrames = (size(handles.vidA,4)-nFrames):size(handles.vidA,4);
    else
        curFrames = (curFrame-(nFrames-1)/2):(curFrame+(nFrames-1)/2);
    end
    
    m = double(squeeze(handles.vidA));
    m = mean(m(:,:,curFrames),3);
    m = (m-min(m(:)))/(max(m(:))-min(m(:)));
    
    set(handles.im,'Cdata',m);
    colormap(gray);
    axis off;
end
