...
% Last Modified by GUIDE v2.5 25-May-2016 07:51:20
...
istep = 1;
cprintf('\n');
cprintf('*blue','Scanbox Yeti (v3.0) - by Dario Ringach (darioringach@me.com)\n');
cprintf('blue','Visit the blog at: ')
cprintf('hyperlink','https://scanbox.wordpress.com/\n')
cprintf('\n');
beep;
pause(2);
...
% set(sbt,'String','Control panels'); drawnow;
...
if(sbconfig.ephys)
    %set(sbt,'String','Ephys Device'); drawnow;
...
if(sbconfig.slm)
    %set(sbt,'String','SLM Device'); drawnow;
...
try
    ...
    % delete(10);
    uiwait(errordlg('Cannot communicate with scanbox. Please, (a) reset scanbox, (b) quit matlab, (c) restart matlab, (d) run scanbox','scanbox','modal'));
    exit;
end
...
if(sbconfig.gain_override>0)
    % set(sbt,'String','Set custom gains'); drawnow;
...
if(length(sbconfig.pockels_lut)==256)
    set(sbt,'String','Loading Pockels LUT'); drawnow;
...
%set(sbt,'String','Set HSYNC sign'); drawnow;
...
if(isempty(sbconfig.tri_knob))
    ...
    set(sbt,'String','3D Mouse'); drawnow;
    import mouse3D.*
    global mouseDrv;
...
%set(sbt,'String','Opening UDP'); drawnow;
    ...
    for(i=0:3)
        ...
        switch i
            ...
            case {1,2}
                ...
                r = tri_send('SAP',4,i,1600);    %% max vel accel - was 2000
                r = tri_send('SAP',5,i,1600);
        end
...
% imaqmem(sbconfig.imaqmem);       % set 4GB of memory. discontinued

% set(sbt,'String','Setting up cameras'); drawnow;
cprintf('*comment','[%02d] Setting up cameras\n',istep); istep=istep+1;
...
%     case 1
%         sbconfig.nroi_auto_size = 64;
%     case 2
%         sbconfig.nroi_auto_size = 64+16;
%     case 3
%         sbconfig.nroi_auto_size = 64+32;
% end
...
% if(handles.linkcheck)
%     global opto2pow
%     for(i=1:size(opto2pow,1))
%         sb_current(opto2pow(:,1),opto2pow(:,2));
%     end
% end
...
if(mm_flag)
    ...
    mmfile.Data.header(1) = -1;
    mmfile.Data.header(2) = nlines;
    mmfile.Data.header(3) = ncol;
end
...
while ~captureDone
    ...
    if bufferFull
        ...
        if(scanmode)
            alazarReshapeCData2(bufferOut.Value,preIdx,chAB,chA,chB,outCData,ttlflagnew,uint16(lines),uint16(get(pmtdisp_h,'Value')));
        else
            alazarReshapeCData2bi(bufferOut.Value,preIdx,postIdx,postIdxA,cdIdx,chAB,chA,chB,outCData,ttlflagnew,uint16(lines),uint16(get(pmtdisp_h,'Value')),uint16(length(S)));
        end
                
        ttlflagnew = bitand(bufferOut.Value(1),3);
        ...
        if(mm_flag)
            if(mmfile.Data.header(1)<0)    % data was consumed
...
if(sbconfig.gpu_pages>0)
    cprintf('*comment','[%02d] Resetting/clearing GPU\n',istep); istep=istep+1;
