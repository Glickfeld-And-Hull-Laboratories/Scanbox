...
int zsflag = 0; // zstack flag
int lock = 0; // are screen and knobs locked?
const int order[4] = {2, 1, 0, 3};

int IRQcount = 0;   // frame counter
int IRQperiod = 30; // how often to step
int deltaz = 0;     // how many steps to simulate in z-stack
...
void update_axis(int n, long val) {
  ...
  tft.setCursor(30, 60 + order[n] * 25); // was 70 30
...
void update_zstack() {
  tft.setCursor(30, 60 + 4 * 25); // was 70 30
  if(zsflag)
    tft.setTextColor(ILI9341_RED, ILI9341_BLACK);
  else
    tft.setTextColor(ILI9341_YELLOW, ILI9341_BLACK);
  format("z = (%d, %d)",IRQperiod,deltaz);
  tft.print(buf);
}
...
void IRQcounter() {
  IRQcount++;
  if (IRQcount % IRQperiod == 0) {
    z.write(z.read() + deltaz);          //simulate change in z
  }
}
...
void loop() {
  ...
  if (lock == 0) {
    ...
    if (rflag == 0 && sflag == 0 && zflag == 0) { // if NOT in store / recall / zero mode
      ...
      if (xs < 200) {                   // clicked outside the selection menu...
        sflag = zflag = rflag = 0;
